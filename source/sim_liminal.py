...
def inside_frame(fr, eye, p):
    """規則「枠に収める」: 目と枠の 4 隅が作る四角錐の中に点 p があるか。
    ★実行時(Liminal.lua の insideFrame)と必ず同じ式にしておくこと。"""
    s0 = None
    for i in range(4):
        a, b = fr[i], fr[(i + 1) % 4]
        ax, ay, az = a[0] - eye[0], a[1] - eye[1], a[2] - eye[2]
        bx, by, bz = b[0] - eye[0], b[1] - eye[1], b[2] - eye[2]
        nx = ay * bz - az * by
        ny = az * bx - ax * bz
        nz = ax * by - ay * bx
        d = nx * (p[0] - eye[0]) + ny * (p[1] - eye[1]) + nz * (p[2] - eye[2])
        if s0 is None:
            s0 = (d >= 0.0)
        elif (d >= 0.0) != s0:
            return False
...
def framed(c, eye):
    """破片の【浮遊姿勢】の 8 隅が全部 枠に収まっているか(実行時の framedAll と同じ)。"""
    for sh in c.shards:
        k = sh["k"]
        F = sh.get("focus") or c.focus
        for p in sh["pts"]:
            q = (F[0] + k * (p[0] - F[0]), F[1] + k * (p[1] - F[1]), F[2] + k * (p[2] - F[2]))
            if not inside_frame(c.frame, eye, q):
                return False
    return True


def shadow_ok(sd, eye):
    """規則「影を重ねる」: 灯りから見た自分の投影が、床の輪郭の中に落ちているか。
    ★実行時(Liminal.lua の shadowHits)と必ず同じ式にしておくこと。"""
    L = sd["L"]
    dy = eye[1] - L[1]
    if dy > -0.20:
        return False
    ...
    sx = L[0] + t * (eye[0] - L[0])
    sz = L[2] + t * (eye[2] - L[2])
    dx, dz = sx - sd["c"][0], sz - sd["c"][1]
    ca, sa = math.cos(math.radians(sd["yaw"])), math.sin(math.radians(sd["yaw"]))
    u = dx * ca - dz * sa
    v = dx * sa + dz * ca
    return abs(u) <= sd["w"] / 2.0 and abs(v) <= sd["l"] / 2.0
...
def conn_error(c, eye, only=None):
    ...
    # ★枠 / 影 は【立ち位置そのものの足切り】なので、誤差の側で 999 にして表す。
    #   こうしておくと field()・[5] 印の検査・calib_marks の全部が同じ規則を見る
    #   (別々の式で書かない、がこの作品の掟)。
    if getattr(c, "frame", None) and not framed(c, eye):
        return 999.0
    if getattr(c, "shadow", None) and not shadow_ok(c.shadow, eye):
        return 999.0
    if c.touch:
...
def main():
    ...
    PLAN = {
        ...
        6: ((17.0, 111.2, 6.00), (17.0, 111.2, 6.00)),   # テラス(振り返る) -> 南壁の上
        ...
        # ---- 第三幕。gen_liminal.ACT3_SMALL でどちらの版かが決まる ----
    }
    PLAN.update({
        # 旧版「立坑」(58x42m・高さ 28m)
    ...
    } if not G.ACT3_SMALL else {
        # 作り直し版(1 部屋 1 動詞)。床は L0=Y3 / L1=+0.90 / L2=+1.80 の 3 段。
        #   11 枠に収める   R1(z172..186, L0) → 段が生えて R2(L1) へ
        #   12 なぞる       R2(z186..201, L1) → 歩廊(L2)が架かって R3 へ
        #   13 影を重ねる   R3(z201..210, L2) → シャッターが沈んで廊下へ
        #   14 枠で消す     廊下(x30..56, L2) → 突き当りの壁が消えて R5 へ
        #   15/16 選ぶ      R5(x56..72, L2) → 二つの井戸のどちらかへ橋が架かる
        # ★ 15 と 16 は多義(excl)。机上検査は両方を順に適用するので、
        #   16 の「解く前に行けてはいけない点」は 15 で既に見ている。None にする。
        11: ((23.0, 188.0, 6.70), (23.0, 188.0, 6.70)),   # 段 -> 0.90m 上の R2 へ
        12: ((29.0, 195.0, 7.60), (29.0, 195.0, 7.60)),   # 歩廊 -> 東壁沿いを北へ
        13: ((32.0, 205.2, 7.60), (32.0, 205.2, 7.60)),   # シャッターが沈んで廊下へ
        14: ((58.0, 205.2, 7.60), (58.0, 205.2, 7.60)),   # 壁が消えて R5 へ
        15: ((69.0, 211.5, 7.60), (69.0, 211.5, 7.60)),   # 東の井戸を渡る
        16: (None, (59.0, 211.5, 7.60)),                  # 西の井戸を渡る
    })
    PLAN.update({
    ...
    for c in order:
        ...
        for m in c.movers:
            w2.disable(m["n"])
    r3, _, _ = walk(w2, start, {"t": (22.0, 137.0, 3.40)}, B)
    if r3["t"]:
    ...
    PTS = [(0, 1.5, -7), (0, 1.5, 0), (0, 1.5, 12), (0, 1.5, 18), (-6.3, 1.5, 18.3),
           ...
           # ---- 第三幕(作り直し版。床 5.80 / 6.70 / 7.60 → 目 +1.70) ----
           (22.0, 7.5, 174.0), (23.0, 7.5, 179.0), (23.0, 7.5, 184.5),
           (23.0, 8.4, 188.0), (20.6, 8.4, 193.6), (27.0, 8.4, 198.5),
           (29.0, 9.3, 195.0), (22.6, 9.3, 205.6), (26.0, 9.3, 208.0),
           (34.0, 9.3, 205.2), (42.0, 9.3, 205.2), (52.0, 9.3, 205.2),
           (60.0, 9.3, 200.0), (66.3, 9.3, 202.6), (64.0, 9.3, 211.5),
           (68.0, 9.3, 217.0),
