...
CHECKS = [
    ...
    # ---- 第三幕(作り直し版)。床は 3 段: L0=5.80 / L1=6.70 / L2=7.60。
    #   y は【体の中心】= 床 + 0.90。
    # ★★到達点どうしの高さが 2.0 以上離れていると、普通に降りただけで
    #   「落ちた」と誤判定されて引き戻される。ここは段差を全部 0.90 にしてある。
    # ★井戸(R5)は 2.80m 掛けてあるので、落ちれば必ずここへ戻される。
    dict(x=22.00, y=6.70, z=174.0, r=2.4),      # 入口(枠の廊下)
    dict(x=23.00, y=6.70, z=179.0, r=2.6),      # 枠の手前
    dict(x=23.00, y=6.70, z=184.0, r=2.4),      # 段の下
    dict(x=23.00, y=7.60, z=187.6, r=2.6),      # 並びの間へ上がった所
    dict(x=20.60, y=7.60, z=193.6, r=2.6),      # 並びの間の焦点
    dict(x=29.00, y=8.50, z=195.0, r=2.4),      # 歩廊の上
    dict(x=28.60, y=8.50, z=200.8, r=2.2),      # 歩廊の北端
    dict(x=24.00, y=8.50, z=203.4, r=2.6),      # 影の間
    dict(x=22.60, y=8.50, z=205.6, r=2.4),      # 影の焦点
    dict(x=32.00, y=8.50, z=205.2, r=2.4),      # 廊下 西
    dict(x=42.00, y=8.50, z=205.2, r=2.6),      # 廊下 中(枠で消す焦点)
    dict(x=52.00, y=8.50, z=205.2, r=2.6),      # 廊下 東
    dict(x=60.00, y=8.50, z=201.5, r=2.6),      # 二つの井戸の間
    dict(x=66.30, y=8.50, z=202.6, r=2.4),      # 選ぶ印
    dict(x=64.00, y=8.50, z=211.5, r=3.0),      # 橋を渡った先
    dict(x=68.00, y=8.50, z=213.0, r=2.4),      # 出口の床
...
MARKS = {
    ...
    "C11_mark": (0.54, 1.34, 0.0),
    "C12_mark": (0.59, 0.84, 86.0),
    "C13_mark": (0.59, 0.94, 85.7),
    "C14_mark": (0.34, 1.44, 86.0),
    "C15_mark": (0.26, 0.84, 48.8),
    "C16_mark": (0.49, 1.04, 121.0),
...
class Conn:
    ...
    def __init__(self, cid, focus, lock, warn, center, note="",
                 ...
                 lens=None, frame=None, shadow=None):
        self.cid = cid
        self.focus = focus
        ...
        self.occl = occl
        # ★★規則「枠に収める」(2026-09-09 第三幕の作り直しで追加)。
        #   frame = [[x,y,z] x 4]。枠の 4 隅を【周回順】で渡す(左下→右下→右上→左上)。
        #   目と 4 隅が作る四角錐の中に、破片の浮遊姿勢が【全部】入っている位置でだけ成立する。
        #   ★回る筒のスリットを「時間の窓」から【空間の窓】へ直したのと同じ考え方の一般形。
        #     筒が円筒の窓なら、これは床に立った四角い窓。判定は 20 行の内積だけ。
        #   ★枠より手前に浮いている破片は、近づくと枠からはみ出す ＝
        #     【前後に歩くことに初めて意味が出る】(今までの 24 本は前後が誤差の微調整だった)。
        #   ★枠は通り道の真ん中に立てること。避けて通れるが、通ろうとして必ず一度は覗く。
        self.frame = [[round(v, 4) for v in q] for q in frame] if frame else None
        # ★★規則「影を重ねる」。この作品で唯一【照準を使わない】足切り。
        #   shadow = dict(L=[灯りの座標], fy=床の高さ, c=[cx,cz], w=, l=, yaw=, board=影板の名前)
        #   灯りから見た自分の投影が、床に描かれた輪郭の中に入る位置でだけ成立する。
        #   影そのものは Liminal.lua が薄い黒板 1 枚で描く(castShadows は使わない)。
        self.shadow = shadow
    ...
    def data(self):
        return dict(id=self.cid, focus=[round(v, 4) for v in self.focus],
                    ...
                    frame=self.frame, shadow=self.shadow)
...
def act2(Y2, DW, DH):
    ...
    # ---- 継ぎ目 09: 大室の向こう岸へ渡す段 ----
    # ★★2026-09-09 ここには「スリットの開いた筒がゆっくり回り、一周に一度だけ
    #   中の破片が見える」仕掛けを置いていた。判定は直したが、遊ぶ側から見ると
    #   【合わせ終えたあと、ただ待つ】時間が一周 13.8 秒ぶん残る。
    #   この作品の手触りは「歩いて、見て、気づく」であって、待つことではない。
    #   待ちを強いる動詞はここ一つだけで、第二幕の締めくくりとしても浮いていた。
    #   筒ごと外し、素直な相似の継ぎ目に戻す。
    #   ★大室そのもの(穴・灯り・南北の壁)は残るので、部屋の広さと道順は変わらない。
    #   ★遮る物が無くなったぶん易しくなるが、ここは第三幕の戸口の直前 ──
    #     大きな戸口(継ぎ目10)の前のひと息として、易しくてよい。

    F9 = (3.20, EYEY, 146.00)
    c9 = Conn(9, F9, 2.5, 14.0, (22.0, Y2 + 1.2, 151.0), "far-stair")
    ...
    mark("C9_mark", (F9[0], Y2 + 0.008, F9[2]), T_TILEF, (0.72, 0.72, 0.70), rough=0.4,
         thick=0.016)
    # ★灯は【浮遊している所】へ置く。破片の縁が拾えないと重ねようが無い
    ...
    # ---- 壁の落書き(継ぎ目 10 の解き方) ----
    # ★ここだけ【入って正面の壁に、大きめに】。理由: この白い部屋は
    #   「行き止まりに見える四角い部屋」で、しかも枠が壁と同じ色に溶けているので、
    #   何をする部屋なのかの取っ掛かりが一つも無い。第四幕・第五幕のように
    #   継ぎ目の脇へ小さく置くと、そもそも見つけてもらえない。
    # ★南の戸口(x=22)から入って北を向くと、この壁が 13.7m 先の正面に来る。
    #   x を 17.6 にしてあるのは、床の印(F10 の x=18.48)と【同じ側】に置くため。
    #   絵を見上げてから視線を下ろすと、その先に印がある。
    # ★北の壁の開口(x=22 / 幅 1.10)と非常口の標識には掛からない(4.4m 離してある)。
    # ★幅 2.10m = 終盤の落書き(1.24〜1.44m)より一回り大きい。13.7m 先から読ませるため。
    # ★真下の歩ける面はこの部屋の床(SILL2)。下端 = SILL2 + 1.97 で、
    #   add_walk_colliders の 1.80m より上 ＝ 当たり判定は付かない(graffiti() の注記)。
    # ★★遠くから読ませるための調整(実機で 3 回撮り直した)。
    #   ・線を終盤の 9 枚の 1.5 倍に太らせた(blender_graffiti.py の gf_a2_gate)。
    #     細い線は 13.7m 先だとミップで縁取りと混ざり、ただの灰色の染みになる。
    #   ・縁取りを終盤(0.035)より濃い 0.07 にした。壁が真っ白なので遠目に効くのは
    #     【暗い塊がどれだけあるか】。芯を炭色にして縁取りを白くする裏返しも試したが、
    #     面積の広い縁取りが白いせいで余計に壁へ沈んだので却下。
    #     ★真っ黒(0.015)まで落とすと、近くで見た時に線が【中抜きの管】に見える。
    #       遠目の効きはほとんど変わらないので、近くの見え方を採って 0.07。
    graffiti("C10_gf", 17.60, SILL2 + 2.72, ZZ1, -1.3, "gf_a2_gate.png", 2.10,
             ink=0.94, scuff=0.20, halo=0.07)
...
def act4(Y4, DW, DH):
    ...
    # 落書き: 視野の【真ん中】に据えると決まらない / 【端】で決まる。
    # ★西の壁(内法 x=56.0)。南の回廊を西へ歩いて北へ折れる、その正面に当たる。
    ...
    # 落書き: 柱の頭と板の先端が【画面の上で】重なるまで歩く(印は無い)。
    # ★東の壁(内法 x=82.0)。東の回廊を北へ歩く人の右手に来る。
    graffiti("C19_gf", GX1, Y4 + GF_Y, 234.60, 91.7, "gf_a4_touch.png", 1.30,
             col=GF_GREASE, ink=0.84, scuff=0.40)
    ...
    # 落書き: 1 つの立ち位置で左右 2 方向を同時に満たす。隣の印では決まらない。
    # ★南の壁(内法 z=222.3)。入口から西へ歩くと、印(x=63.5)の手前の左手に来る。
    graffiti("C20_gf", 61.60, Y4 + GF_Y, GZ0, 178.2, "gf_a4_both.png", 1.30, scuff=0.26)
    ...
    # 落書き: 明かりが点いている間は決まらない。消えた一瞬だけ。
    # ★北の壁(内法 z=246.0)の東寄り。ここを照らすのは【明滅バンク】なので、
    #   落書き自身が明滅する壁に載る = 絵の言っていることが目の前で起きる。
    #   (落書きは灯りに依らないシェーダーなので、暗い一瞬でも読める)
    graffiti("C21_gf", 72.60, Y4 + GF_Y, GZ1, 2.3, "gf_a4_dark.png", 1.30,
             col=GF_GREASE, ink=0.86, scuff=0.36)
    ...
    # 落書き: 4 つの印を歩いて回り、印ごとに階段が 1 群ずつ増える。
    # ★北の壁(内法 z=246.0)の西寄り。階段(x=68)の左隣なので、
    #   組み上がっていく階段と同じ画面に入る。21 の落書き(x=72.6)からは 9m 離す。
    graffiti("C17_gf", 63.40, Y4 + GF_Y, GZ1, -1.8, "gf_a4_visit.png", 1.34, scuff=0.32)
    plight("C17_gfl", (63.40, Y4 + 3.20, GZ1 - 1.10), WARM, 5.0, 5.0)
...
def act5(Y5, DW, DH):
    ...
    # 落書き: 偽の板を配管の陰へ入れる → 井戸に蓋。連鎖の輪 1/4。
    # ★壁ではなく【柱の南面】に書く。入口(x=68)から北を向いた真正面 4m にある柱で、
    #   この部屋で一番最初に目が行く面。柱幅 1.4m に収まるよう幅は 1.16m。
    graffiti("C22_gf", 68.0, Y5 + 2.32, 262.0 - 0.70, -1.6, "gf_a5_hide.png", 1.16,
             scuff=0.28)
    plight("C22_gfl", (68.0, Y5 + 3.10, 260.20), WARM, 5.0, 5.0)
    ...
    # 落書き: 浮いている段を【床の影】へ落とす → 棚への上がり口。連鎖の輪 2/4。
    # ★東の壁(内法 x=86.0)。井戸を渡って北へ出た人が、棚(x=74..80)を見る向きの
    #   その先に当たる。遠くから読ませるので 1 枚だけ大きめ(1.44m)。
    graffiti("C23_gf", X1, Y5 + 2.42, 293.60, 88.7, "gf_a5_shadow.png", 1.44,
             col=GF_GREASE, ink=0.90, scuff=0.24)
    plight("C23_gfl", (84.20, Y5 + 3.30, 293.60), WARM, 6.0, 6.5)
    ...
    # 落書き: 印は 3 つ・地上からでは決まらない → 壁ぎわに段。連鎖の輪 3/4。
    # ★北の壁の【棚の真上】。棚に上がった人の目(棚 +1.70)のすぐ上 3m 先に来る。
    #   ★真下の歩ける面が棚(+0.60)なので、下端は棚 +1.80 より上 = 中心 Y5+3.00。
    #     床の高さで計算すると当たり判定が付いて【見えない壁】になる。
    #   ★x は 77.4 ではなく 75.3。真上に非常口の標識(W5_lure2 = x 77.0)があって、
    #     緑の標識と落書きが上下でくっついて見えた(実機で確認して西へずらした)。
    graffiti("C24_gf", 75.30, Y5 + 3.00, Z1, -2.1, "gf_a5_marks.png", 1.24, scuff=0.30)
    ...
    # 落書き: 4 方向を同時に + 明かりが落ちた一瞬 → 北壁が割れて開く。連鎖の輪 4/4。
    # ★北の壁、出口(x=54.9〜57.1)の東隣。踊り場に上がった人の目(踊り場 +1.70)の
    #   すぐ上に来る。★真下の歩ける面が踊り場(+2.40)なので中心は Y5+4.84。
    # ★ここを照らすのは仕上げの明滅バンク。落書きは灯りに依らないので、
    #   待っている「暗い 1 秒」の間もこの絵だけは読めたままになる。
    graffiti("C25_gf", 58.70, Y5 + 4.84, Z1, 1.9, "gf_a5_four.png", 1.24,
             col=GF_GREASE, ink=0.92, scuff=0.22)
...
SIGN_Y = 2.24              # 案内板の中心の【床からの】高さ。★下端 1.91m は身長 1.80 より上
SIGN_W, SIGN_H = 1.00, 0.66


def guide_sign(name, x, y, z, yaw, png):
    """壁に貼る案内板 1 枚。額縁(模型) + 絵の板(箱) + 小さな灯り。

    x, z … 貼る壁の【面】の位置(壁の中心ではない)。y … 板の中心の高さ(絶対値)。
            幕ごとに床の高さが違うので、呼ぶ側で 床 + SIGN_Y を渡すこと。
    yaw … 板が向く向き。0=-Z を向く / +90=-X / 180=+Z / -90=+X。
            (箱は -Z 面へ絵を貼る作法。exit_sign_x と同じ)

    ★面が出ている向きへ 0.045 ずらして絵を出す。揃えないと絵だけ壁へ潜り、
      机上検査[6](同一平面)に引っかかる。
    ★★当たり判定は付けない(壁に貼った飾りで、壁側に当たり判定がある)。
      add_walk_colliders() は「床から p_height=1.80 より上にある箱」を対象外に
      するので、板の下端(y - 0.33)が床 +1.80 より上にあることが条件。
      SIGN_Y=2.24 なら下端は床 +1.91。高さを下げるなら、その規則の側も見直すこと。
      ついでに、この高さは【施設の案内板の高さ】そのものでもある(目線 1.70 の少し上)。
    """
    # 板が向く向きの単位ベクトル。★軸に乗る yaw で -0.0 が出ないよう丸めて 0 を足す
    fx = round(-math.sin(math.radians(yaw)), 9) + 0.0
    fz = round(-math.cos(math.radians(yaw)), 9) + 0.0
    # 額縁。原点 = 壁の面の中心で、-Z が見る側。倍率は 1(実寸で作ってある)
    fr = ent(name + "_fr", (x, y, z), (1, 1, 1), (0, yaw, 0))
    fr["meshRenderer"] = dict(modelPath="models/props/sign_guide.gltf")
    # 絵の板。額縁の裏の決り(しゃくり)へ落とし込む。
    # ★Blender で models/props/sign_guide.gltf を実測した内法(2026-09-09):
    #     手前の面 0.088 … 1.008 x 0.670(面取り)
    #     見付の内 0.075 … 0.980 x 0.640
    #     裏の決り 0.030 … 0.905 x 0.565  ← ここが絵を隠す
    #   板の面(0.060)の高さで見ると、見えるのは概ね 0.955 x 0.615 =
    #   1000x660 の絵の【四辺 22px ほどが額縁に隠れる】。
    #   だから SVG は四辺 25px より内側にだけ絵を描くこと(枠線 16px は隠れてよい)。
    # ★uvTiling は 1x1。焼いた PNG を 1 枚だけ貼る(繰り返さない)
    pl = box(name, (x + fx * 0.045, y, z + fz * 0.045), (SIGN_W, SIGN_H, 0.03), png, "z",
             rough=0.86, tile=(1.0, 1.0))
    pl["transform"]["rotation"] = [0, yaw, 0]
    # 板を照らす小さな灯り。★弱く(0.9)。強いと壁に白い染みが出て、
    #   「飛び飛びに点いた蛍光灯」で作った明暗の縞が消える
    plight(name + "_l", (x + fx * 0.315, y + 0.22, z + fz * 0.315), WARM, 0.9, 2.4)


# ---------------------------------------------------------------- 壁の落書き(終盤)
GF = "ui/graffiti/"
GF_ASPECT = 1400.0 / 1000.0     # 焼いた PNG の縦横比(source/blender_graffiti.py の画布)
GF_CHALK = [0.78, 0.76, 0.70]   # 白いチョーク
GF_GREASE = [0.70, 0.67, 0.61]  # 古い・脂っぽい線
GF_HALO = 0.035                 # 線の縁取りの暗さ。明るい壁でも形が読めるようにする
# 幅 1.30m(= 高さ 0.93m)の落書きの中心の【真下の歩ける面からの】高さ。
# ★下端 = +1.90m。add_walk_colliders の 1.80m より上 = 当たり判定が付かない(下の注記)。
#   案内板(SIGN_Y=2.24 / 下端 1.91)とほぼ同じ帯。幅を変えるなら計算し直すこと。
GF_Y = 2.36


def graffiti(name, x, y, z, yaw, png, w=1.30, col=None, ink=0.88,
             scuff=0.34, scuff_scale=9.0, off=0.045, halo=GF_HALO):
    """壁に直接なぐり書きした落書き 1 枚。額縁も下地も無い【透過の絵だけ】。

    x, z … 貼る壁(または柱)の【面】の位置。y … 絵の中心の高さ(絶対値)。
    yaw … 0=-Z を向く / +90=-X / 180=+Z / -90=+X。guide_sign と同じ作法。
    w   … 横幅[m]。高さは PNG の縦横比から決める(潰れないように)。

    ★案内板(guide_sign)との違いは 3 つ:
        1. 額縁(models/props/sign_guide.gltf)を付けない。掲示物ではなく書き置き
        2. 絵は背景透過。assets/shaders/Graffiti.hlsl がアルファで抜いて線だけ描く
        3. yaw を数度ずらし、大きさと濃さと擦れを 1 枚ずつ変える

    ★★高さの決まり(これを外すと【見えない壁】ができる)
      add_walk_colliders() は「primitive=box + テクスチャ + 当たり判定なし」を
      片っ端から掴んで当たり判定を足す。落書きも例外ではないので、
      【絵の下端が、真下の歩ける面から 1.80m 以上】でなければならない。
      幕ごと・場所ごとに真下の面の高さが違う(棚の上・踊り場の上)ので、
      呼ぶ側がそこを計算して y を渡すこと。目安は 真下の面 + 1.80 + 高さ/2 + 0.10。
      ★同じ規則が source/fix_colliders.js にもある。名前で例外を作ると片方だけ
        直すことになるので、【高さで守る】方を選んでいる。

    ★★向きの決まり
      絵は既に 180 度回して焼いてある(source/blender_graffiti.py の ROT180)。
      ここでは辻褄合わせを【一切しない】。両方でやると元に戻る。

    ★当たり判定は付けない(壁に書いた線で、壁側に当たり判定がある)。
    ★面から 0.045 浮かせる。案内板と同じ値で、机上検査[6](同一平面)にも掛からない。
      yaw を数度ずらすので、板の隅は最大 0.65*sin(2.5度)=0.03m しか動かない ＝ 壁へ潜らない。
    """
    h = round(w / GF_ASPECT, 4)
    fx = round(-math.sin(math.radians(yaw)), 9) + 0.0
    fz = round(-math.cos(math.radians(yaw)), 9) + 0.0
    e = box(name, (round(x + fx * off, 4), y, round(z + fz * off, 4)), (w, h, 0.012),
            GF + png, "z", rough=0.95, tile=(1.0, 1.0))
    e["transform"]["rotation"] = [0, yaw, 0]
    e["color"] = [1, 1, 1]
    e["shader"] = "Graffiti.hlsl"
    e["shaderAlphaBlend"] = True
    e["shaderEffectValue"] = scuff                       # 擦れ
    # むらの細かさ / 四辺で消す幅 / 縁取りの暗さ
    # ★PNG は【芯(赤=1) + 太らせた縁取り(赤=0)】の 2 層で焼いてある。
    #   縁取りが無いと、明滅バンクの真下のような明るい壁で白い線が消える(実機で確認)。
    e["shaderParamsB"] = [scuff_scale, 0.05, halo]
    c = col or GF_CHALK
    e["shaderParams"] = [c[0], c[1], c[2], ink]
    return e
...
def build():
    ...
    # ★関数は module 直下(guide_sign)。第三幕からも同じ物を貼るため。
    #   第一幕の廊下は 東の壁 x=+1.5(yaw=+90) / 西の壁 x=-1.5(yaw=-90)、床は 0。
    ...
    guide_sign("A_sign1", +1.5, SIGN_Y, -1.60, +90.0, "ui/signs/sign_walk_look.png")
    guide_sign("A_sign2", -1.5, SIGN_Y, 3.40, -90.0, "ui/signs/sign_stand_mark.png")
    guide_sign("A_sign3", +1.5, SIGN_Y, 11.40, +90.0, "ui/signs/sign_shards_align.png")
    ...
    # 【どこかで鳴っている音楽】の器。中身は assets/components/StageMusic.lua に全部ある。
    # ★空の実体で位置も持たない。音源の座標はスクリプトが自分で持っていて、
    #   playSpatialId / moveVoice で幕をまたいで漂わせる。ここは載せ台でしかない。
    # ★LM_ 始まりなので group_entities が「システム」へ入れる。
    mz = ent("LM_Music")
    mz["luaScript"] = dict(enabled=True, scriptPath="components/StageMusic.lua")
