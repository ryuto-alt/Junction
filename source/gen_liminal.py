...
def act3(Y3, DW, DH):
    ...
    PD = (GZ1 - GZ0) / 3.0
    # ★出来上がる物の見た目を散らす(自己採点で「橋」が 10/25 本もあった)。
    #   ここは【幅 1.0m の細い梁】。偽物の板は 0.62〜2.37m 幅で散っているので、
    #   細くしても「本物だけ形が違う」にはならない = 見分けの難しさは保たれる。
    for i, k in enumerate((0.42, 0.55, 0.68)):
        zc = GZ0 + PD * (i + 0.5)
        pl = box("C11_p%d" % i, (22.0, Y3 - 0.08, zc), (1.0, 0.16, PD), T_METAL, "y",
                 rough=0.5, metal=0.45, color=[0.50, 0.50, 0.47])
        c11.shard(k, [pl])
        c11.solid(hit("C11_h%d" % i, (22.0, Y3 - 0.08, zc), (1.1, 0.16, PD + 0.30),
                      kinematic=True))
    ...
    # ★ここは【空調ダクトの上を渡る】。板を並べるのは第一幕・第二幕でもう 2 回やっている。
    #   天端は Y4 のまま(歩く高さは変えない)。厚み 0.85m の角ダクトが空を横切る絵になる。
    DKH = 0.85
    box("C12_anchor", (20.5, Y4 - DKH / 2, QDZ), (2.2, DKH, CWD), T_METAL, "y", rough=0.5,
        metal=0.5, color=[0.50, 0.50, 0.47], solid=True)
    for zr in (QDZ - CWD / 2 + 0.10, QDZ + CWD / 2 - 0.10):
        box("C12_anchor_r%.1f" % zr, (20.5, Y4 - DKH / 2, zr), (2.24, DKH + 0.04, 0.09),
            T_METAL, "z", rough=0.45, metal=0.6, color=[0.44, 0.44, 0.42])
    for i, k in enumerate((0.45, 0.58, 0.70, 0.82)):
        ...
        xc = 21.6 + 2.2 * i + w12 / 2
        pl = box("C12_p%d" % i, (xc, Y4 - DKH / 2, QDZ), (w12, DKH, CWD), T_METAL, "y",
                 rough=0.5, metal=0.5, color=[0.50, 0.50, 0.47])
        rb = box("C12_r%d" % i, (xc - w12 / 2 + 0.12, Y4 - DKH / 2, QDZ),
                 (0.09, DKH + 0.04, CWD + 0.04), T_METAL, "x", rough=0.45, metal=0.6,
                 color=[0.44, 0.44, 0.42])
        g = glow("C12_g%d" % i, (xc, Y4 + 0.01, QDZ - CWD / 2 + 0.05), (w12 - 0.1, 0.04, 0.06),
                 GOLD, 1.25)
        c12.shard(k, [pl, rb, g], glows=[g])
    ...
    box("R1_le", (43.65, Y4 - WT / 2, 195.75), (5.3, WT, 6.5), T_METAL, "y",
        rough=0.5, metal=0.5, color=[0.48, 0.48, 0.46], solid=True)
    # ★ここは【成の深い梁】。平たい板は第一幕・第二幕・継ぎ目12 で見飽きている。
    #   k>1 の破片なので、吹き抜けに浮かぶ方は 6〜8m の【巨大な鉄骨】に見える。
    BMH = 0.75
    box("C13_anchor", (40.25, Y4 - BMH / 2, RDZ), (1.9, BMH, 2.0), T_METAL, "y", rough=0.5,
        metal=0.5, color=[0.50, 0.50, 0.47], solid=True)
    ...
    for i, (xc, w3, k) in enumerate(((34.7, 2.0, 2.40), (36.8, 2.2, 1.90), (38.6, 1.4, 1.55))):
        pl = box("C13_p%d" % i, (xc, Y4 - BMH / 2, RDZ), (w3, BMH, 2.0), T_METAL, "y",
                 rough=0.5, metal=0.5, color=[0.50, 0.50, 0.47])
        g = glow("C13_g%d" % i, (xc, Y4 + 0.01, RDZ - 0.96), (w3 - 0.1, 0.04, 0.06), GOLD, 1.25)
        c13.shard(k, [pl, g], glows=[g])
        c13.solid(hit("C13_h%d" % i, (xc, Y4 - BMH / 2, RDZ), (w3 + 0.4, BMH, 2.0),
                      kinematic=True))
...
def act4(Y4, DW, DH):
    ...
    for i, k in enumerate((0.40, 0.58)):
        ...
        pl = box("C20_p%d" % i, (68.0, Y4 + 0.14, zc), (1.1, 0.28, 1.10),
                 T_CONC, "y", rough=0.85, color=[0.57, 0.57, 0.55])
        c20.shard(k, [pl])
