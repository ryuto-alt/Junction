...
CHECKS = [
    ...
    dict(x=77.00, y=12.70, z=299.0, r=2.4),     # 棚の上
    dict(x=56.00, y=14.20, z=299.0, r=2.2),     # 踊り場
]
GOAL = dict(x=53.5, y=13.30, z=302.60, r=0.95, need=25)
...
MARKS = {
    ...
    "C22_mark": (0.54, 0.94, 177.7),
    "C23_mark": (0.44, 1.14, 56.3),
    "C24_mark": (0.49, 0.84, 122.1),
    "C25_mark": (0.54, 0.54, 176.0),
}
...
def act5(Y5, DW, DH):
    ...
    XIT_X, XIT_W, XIT_Y, XIT_H = 53.5, 2.20, Y5 + 2.70, 2.40
    ...
    for lx, lz, on in ((57.0, 261.0, True), (68.0, 261.0, False), (79.0, 261.0, True),
                       (57.0, 268.0, False), (68.0, 268.0, True), (79.0, 268.0, False),
                       (57.0, 288.0, True), (68.0, 288.0, True), (79.0, 288.0, False),
                       (57.0, 296.0, False), (68.0, 296.0, True), (79.0, 296.0, True)):
        ...
        if on:
            glow("W5_lg%.0f_%.0f" % (lx, lz), (lx, Y5 + HH - 0.49, lz), (0.72, 0.05, 0.72),
                 WARM, 1.5)
            plight("W5_ll%.0f_%.0f" % (lx, lz), (lx, Y5 + HH - 1.1, lz), WARM, 15.0, 19.0)

    # ================================================== 継ぎ目 22: かくれて合わせる（遮蔽）
    # ★偽の板が視界にある限り決まらない。柱の陰に入れて【消す】と決まる。
    #   「どこに立つか」に「何を見ないようにするか」が重なる ＝ 今までに無い探し方。
    F22 = (66.80, EY5, 265.60)
    COL = (68.0, 268.0)                                   # 隠すのに使う柱
    LURE = (69.80, Y5 + 1.40, 274.00)                     # 偽の板(絶対に実体化しない)
    c22 = Conn(22, F22, 2.6, 14.0, (68.0, Y5 + 0.4, 278.0), "hide-behind",
               occl=dict(p=[round(v, 3) for v in LURE],
                         boxes=[[COL[0] - 0.7, Y5, COL[1] - 0.7,
                                 COL[0] + 0.7, Y5 + HH, COL[1] + 0.7]]))
    ...
    # ================================================== 継ぎ目 23: 床の影にはめる
    # ★目標が初めて【目に見える】継ぎ目。床に階段の影が描いてあり、浮いている破片が
    #   その影にぴたりと収まる位置を探す。出来上がるのは影の場所ではなく、その先の段。
    F23 = (68.00, EY5, 289.00)
    c23 = Conn(23, F23, 2.4, 13.0, (77.2, Y5 + 0.6, 294.0), "floor-shadow")
    # 登った先の棚(最初から実在。何を組もうとしているかの見本になる)
    box("W5_deck", (77.0, Y5 + 0.60, 299.0), (6.0, 1.20, 6.0), T_METAL, "y", rough=0.5,
        metal=0.5, color=[0.48, 0.48, 0.46], solid=True)
    st23 = []
    for j in range(4):
        st23.append(box("C23_s%d" % j, (77.2, Y5 + 0.30 * (j + 1) - 0.15, 292.85 + 0.90 * j),
                        (2.4, 0.30, 0.90), T_METAL, "y", rough=0.5, metal=0.5,
                        color=[0.50, 0.50, 0.47]))
        c23.solid(hit("C23_h%d" % j, (77.2, Y5 + 0.30 * (j + 1) - 0.15, 292.85 + 0.90 * j),
                      (2.4, 0.30, 1.02), kinematic=True))
    g23 = glow("C23_g", (77.2, Y5 + 1.22, 295.55), (2.3, 0.04, 0.06), GOLD, 1.25)
    ...
    # ================================================== 継ぎ目 24: 三つの印（偽の印 + 連鎖）
    # ★印が 3 つある。2 つは嘘。しかも 22/23 を解いて【棚の上】に立てないと本物も成立しない。
    F24 = (77.00, Y5 + 1.20 + EYE, 299.00)
    c24 = Conn(24, F24, 2.2, 12.0, (68.0, Y5 + 2.1, 301.4), "three-marks",
               needs=(22, 23), min_y=F24[1] - 0.30, max_y=F24[1] + 0.30)
    for j in range(5):
        xc = 72.5 - 3.0 * j
        top = Y5 + 1.50 + 0.30 * j
        pl = box("C24_p%d" % j, (xc, top - 0.11, 301.40), (3.0, 0.22, 1.20), T_METAL, "y",
                 rough=0.5, metal=0.5, color=[0.50, 0.50, 0.47])
        g = glow("C24_g%d" % j, (xc, top + 0.01, 300.85), (2.9, 0.04, 0.06), GOLD, 1.25)
        c24.shard((0.46, 0.54, 0.62, 0.70, 0.78)[j], [pl, g], glows=[g])
        c24.solid(hit("C24_h%d" % j, (xc, top - 0.11, 301.40), (3.2, 0.22, 1.20),
                      kinematic=True))
    # ★遠い対象(西へ 16m の段)だけだと角度が鈍く、確定域が 6.5m2 まで広がって
    #   「どの印でも解ける」になってしまう。棚の際の手すり = 近い対象を 1 つ混ぜて締める。
    rl = box("C24_rail", (79.60, Y5 + 1.75, 299.0), (0.09, 0.09, 5.4), T_METAL, "y",
             rough=0.45, metal=0.7, color=[0.56, 0.56, 0.53])
    rg = glow("C24_railg", (79.60, Y5 + 1.82, 299.0), (0.03, 0.03, 5.0), GOLD, 1.25)
    c24.shard(0.55, [rl, rg], glows=[rg])
    mark("C24_mark", (F24[0], Y5 + 1.208, F24[2]), T_METAL, (0.66, 0.66, 0.63), rough=0.6)
    # 嘘の印(ここでは何も起きない)。★大きさも色も本物と同じにする
    for j, (mx, mz) in enumerate(((74.6, 297.2), (79.4, 300.6))):
        box("C24_dud%d" % j, (mx, Y5 + 1.208, mz), (1.20, 0.012, 1.20), T_METAL, "y",
    ...
    #   確定域は極小になる ＝ この作品で一番難しい一点。しかも 22〜24 が前提。
    box("W5_land", (56.0, Y5 + 1.35, 299.0), (6.0, 2.70, 6.0), T_METAL, "y", rough=0.5,
        metal=0.5, color=[0.48, 0.48, 0.46], solid=True)
    ...
    for bx in (54.0, 58.0):
        ...
        DARK5.append(nm)
    F25 = (56.00, Y5 + 2.70 + EYE, 299.00)
    # ★仕上げは規則を【重ねる】: 4 方向同時(A) × 暗の一瞬(E) × 連鎖(22〜24)。
    #   立ち位置を合わせてから、明かりが落ちる 1 秒を待つ ＝ 一問一答では絶対に解けない。
    c25 = Conn(25, F25, 2.9, 13.0, (55.0, Y5 + 3.6, 301.0), "four-at-once",
               needs=(22, 23, 24), min_y=F25[1] - 0.30, max_y=F25[1] + 0.30,
    ...
    rails = ((0.52, "s", (56.0, Y5 + 3.25, 296.10), (6.0, 0.09, 0.09)),
             (0.60, "w", (53.10, Y5 + 3.25, 299.0), (0.09, 0.09, 6.0)),
             (0.68, "e", (58.90, Y5 + 3.25, 299.0), (0.09, 0.09, 6.0)),
             (0.76, "n", (56.90, Y5 + 3.25, 301.90), (4.2, 0.09, 0.09)))
    ...
    #   (擦れ跡が無いと 6x6m の踊り場で 0.2m2 を探すことになる)
    mark("C25_mark", (F25[0], Y5 + 2.708, F25[2]), T_METAL, (0.60, 0.60, 0.57), rough=0.6)
