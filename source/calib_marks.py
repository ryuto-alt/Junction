...
MIN_MARK_BY = {5: 0.30, 6: 0.25, 7: 0.25, 8: 0.25, 9: 0.30, 17: 0.25,
               22: 0.14, 23: 0.30, 24: 0.30, 25: 0.18}
# 細い管しか無い継ぎ目は、印の【幅】の下限も下げないと lock が上がるばかりになる
# ★★継ぎ目9(回る)の特例を外した(2026-09-09)。
#   以前は「時間の窓(一周 10 秒のうち 0.94 秒)で既に締まっている」ことを理由に
#   印を 0.16 x 0.24 まで小さくしてよいことにしていた。その結果 0.19 x 0.74 =
#   0.141m2 ── 【プレイヤーの体の直径 0.68m より狭い印】になっていた。
#   スリットを 2 本にして時間の窓を空間の窓へ変えたので、締める理由が消えた。
#   人が立てる大きさへ戻す。
MIN_WL_BY = {22: (0.18, 0.26), 24: (0.18, 0.26),
             25: (0.18, 0.26)}
...
#   15/16 も多義(同じ 5 枚の板を西の井戸と東の井戸で取り合う)。
#   shard_display は k から実体を逆算するので、soft を掛けると実体の位置ごと動く。
NO_SOFT = {7, 8, 13, 15, 16, 19}
ISL = {0: "A", 1: "D", 2: "C", 3: "B"}
